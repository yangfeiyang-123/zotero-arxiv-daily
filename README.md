...
| HISTORY_FILE | | str | Optional path for the sent-paper history file used to avoid recommending the same arXiv IDs again across runs. Default is `.state/sent_arxiv_ids.json`. | .state/sent_arxiv_ids.json |
...
To avoid sending the same paper repeatedly when the latest arXiv batch does not change, the workflow stores sent arXiv IDs in `.state/sent_arxiv_ids.json` and filters them out in later runs. In GitHub Actions, this state is persisted via the Actions cache in the main workflow.
